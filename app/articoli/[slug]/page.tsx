...
>  
      ...
      <div style={{ position: 'fixed', top: 0, left: 0, right: 0, height: 3, background: 'rgba(255,255,255,0.06)', zIndex: 999 }}>
        <div style={{
          height: '100%',
          width: `${readProgress}%`,
          background: 'linear-gradient(to right, #8fd3ff, #4fa8e8)',
...
>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
            <button
              onClick={() => router.back()}
              style={{ width: 34, height: 34, borderRadius: 10, background: 'rgba(255,255,255,0.06)', border: '0.5px solid rgba(255,255,255,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#fff', fontSize: 14 }}
            >←</button>
            {categoria && (
              <span style={{ fontSize: 9, letterSpacing: 3, textTransform: 'uppercase', color: '#8fd3ff', fontWeight: 800 }}>{categoria}</span>
            )}
          </div>
...
/>
          ...
          <div style={{ background: '#0d1829', borderRadius: 22, overflow: 'hidden', border: '0.5px solid rgba(143,211,255,0.12)', marginBottom: 24 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 1, background: 'rgba(255,255,255,0.05)' }}>
              ...
              {/* Sintesi */}
              <button
                onClick={generaSintesi}
                style={{ background: sezione === 'sintesi' ? '#0f2040' : '#0a1220', border: 'none', padding: '18px 16px', cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: 8 }}
              >
                <div style={{ width: 36, height: 36, borderRadius: 12, background: 'rgba(143,211,255,0.08)', border: '0.5px solid rgba(143,211,255,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#8fd3ff" strokeWidth="2">
              ...
              {/* Parole chiave */}
              <button
                onClick={generaParole}
                style={{ background: sezione === 'parole' ? '#0f2040' : '#0a1220', border: 'none', borderTop: '0.5px solid rgba(255,255,255,0.04)', padding: '18px 16px', cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: 8 }}
              >
                <div style={{ width: 36, height: 36, borderRadius: 12, background: 'rgba(143,211,255,0.08)', border: '0.5px solid rgba(143,211,255,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#8fd3ff" strokeWidth="2">
              ...
              <button
                onClick={generaTest}
                style={{ background: sezione === 'test' ? '#0f2040' : '#0a1220', border: 'none', borderTop: '0.5px solid rgba(255,255,255,0.04)', padding: '18px 16px', cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: 8 }}
              >
                <div style={{ width: 36, height: 36, borderRadius: 12, background: 'rgba(143,211,255,0.08)', border: '0.5px solid rgba(143,211,255,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#8fd3ff" strokeWidth="2">
              ...
              <div style={{ padding: '16px 18px', borderTop: '0.5px solid rgba(255,255,255,0.05)', background: '#080f1e' }}>
                ...
                {sezione === 'sintesi' && (
                  <div>
                    ...
                    {loadingSintesi ? <LoadingDots /> : (
                      <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.7)', lineHeight: 1.8 }}>{sintesi}</div>
                    )}
                  </div>
                )}
                    ...
                    {loadingParole ? <LoadingDots /> : (
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                        {paroleChiave.map((p, i) => (
                          <div key={i} style={{ padding: '6px 14px', borderRadius: 99, background: 'rgba(143,211,255,0.08)', border: '0.5px solid rgba(143,211,255,0.2)', color: '#8fd3ff', fontSize: 11, fontWeight: 700 }}>
                            {p}
                          </div>
                        ))}
                      </div>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                              {d.opzioni?.map((op: string, j: number) => {
                                ...
                                return (
                                  <button
                                    key={j}
                                    onClick={() => !risposto && setTestRisposte(prev => ({ ...prev, [i]: j }))}
