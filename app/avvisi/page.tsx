...
function ReelEmbed({ url }: { url: string }) {
  return (
    <a href="https://www.instagram.com/orizzonte.giuridico/reels/" target="_blank" rel="noreferrer"
      style={{ minWidth: 130, borderRadius: 16, overflow: 'hidden', position: 'relative', border: '0.5px solid rgba(255,255,255,0.06)', background: '#111526', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', aspectRatio: '9/16', textDecoration: 'none', flexShrink: 0 }}>
      <div style={{ fontSize: 28, marginBottom: 8 }}>▶</div>
      <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.4)', textAlign: 'center', padding: '0 8px' }}>Vedi i Reels su Instagram</div>
    </a>
  );
}
...
function TikTokEmbed({ url }: { url: string }) {
  return (
    <a href="https://www.tiktok.com/@orizzonte.giuridico" target="_blank" rel="noreferrer"
      style={{ minWidth: 130, borderRadius: 16, overflow: 'hidden', position: 'relative', border: '0.5px solid rgba(255,255,255,0.06)', background: '#111526', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', aspectRatio: '9/16', textDecoration: 'none', flexShrink: 0 }}>
      <div style={{ fontSize: 28, marginBottom: 8 }}>♪</div>
      <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.4)', textAlign: 'center', padding: '0 8px' }}>Vedi i video su TikTok</div>
    </a>
  );
}
...
export default function NormaHomeV2() {
  ...
  useEffect(() => {
    ...
    return () => { clearTimeout(timer); clearTimeout(anim); };
  }, [cur]);
  ...
  async function loadPostsByName(name: string) {
    try {
      ...
      const postsRes = await fetch(`https://orizzontegiuridico.com/wp-json/wp/v2/posts?_embed&categories=${catId}&per_page=3`);
    ...
    <>
      ...
      <style jsx global>{`
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .cats { display: flex; gap: 8px; overflow-x: auto; padding-bottom: 14px; margin-bottom: 6px; scrollbar-width: none; min-height: 90px; align-items: flex-start; flex-wrap: wrap; }
        ...
        .hero-ov1 { position: absolute; inset: 0; background: linear-gradient(to right, rgba(0,15,40,0.92) 0%, rgba(0,15,40,0.65) 50%, rgba(0,15,40,0.1) 100%); }
        .hero-ov2 { position: absolute; inset: 0; background: linear-gradient(to top, rgba(0,15,40,0.8) 0%, transparent 60%); }

        /* PRIMA SLIDE — testo grande che riempie */
        .hero-body {
          position: absolute;
          inset: 0;
        ...
        .hero-wrap img { transition: opacity 0.35s ease; }
      `}</style>

      <div className="app">
        <div className="sb">
          <span className="sb-t">9:41</span>
          <span className="sb-t" style={{ opacity: 0.5 }}>●●●</span>
        </div>
        ...
        <div className="feed">
          ...
          <div className="cats">
            {categories.map((cat, idx) => (
              <button key={idx} className="cat" onClick={() => openStory(cat.name)}
          ...
          <div
            ...
            onTouchEnd={(e) => {
              ...
              if (Math.abs(diff) > 30) {
                e.preventDefault();
                if (diff > 0) setCur((prev) => (prev + 1) % slides.length);
                else setCur((prev) => (prev - 1 + slides.length) % slides.length);
                setFill(0);
              }
            }}
            onWheel={(e) => {
              ...
              if (e.deltaX > 0) setCur((prev) => (prev + 1) % slides.length);
              else setCur((prev) => (prev - 1 + slides.length) % slides.length);
              setFill(0);
            }}
          >
            {slides[cur].img ? (
              ...
              /* SLIDE SENZA IMMAGINE — testo centrato, pieno */
              <div style={{
            ...
            <div className="hero-prog">
              <div className="hero-prog-fill" style={{ width: `${fill}%`, transition: 'width 5s linear' }} />
            </div>
          </div>
            ...
            return (
              <a key={i} href={isOdl ? link : `/articoli/${post.slug}`} target={isOdl ? '_blank' : '_self'} rel={isOdl ? 'noreferrer' : undefined} className="art" style={{ display: 'block', textDecoration: 'none', cursor: 'pointer' }}>
                {img ? (
                  ...
                  <div style={{ height: 130, background: '#07162b', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 28 }}>📄</div>
                )}
            ...
            <>
              {[0, 1, 2].map(i => (
                <div key={i} className="art">
                  <div style={{ height: 130, background: '#07162b', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'rgba(255,255,255,0.2)', fontSize: 11, letterSpacing: 1 }}>CARICAMENTO...</div>
                </div>
              ))}
            </>
          )}

          <a href="/articoli" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '13px 0', borderRadius: 16, background: 'rgba(143,211,255,0.08)', border: '0.5px solid rgba(143,211,255,0.2)', color: '#8fd3ff', fontSize: 10, fontWeight: 800, letterSpacing: 1, textDecoration: 'none', marginBottom: 8 }}>
            VEDI TUTTI GLI ARTICOLI →
          </a>

          {fascicolo && (
            <a href={fascicolo.url} target="_blank" rel="noreferrer" download
              style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, padding: '14px 0', borderRadius: 16, background: 'rgba(168,200,240,0.08)', border: '0.5px solid rgba(168,200,240,0.25)', color: '#a8c8f0', fontSize: 10, fontWeight: 800, letterSpacing: 1, textDecoration: 'none', marginBottom: 8 }}>
      ...
      {storyOpen && (
        <div
          ...
          onClick={(e) => {
            ...
            if (x > w / 2) {
              if (storyPosts.length > 0 && storyIndex < storyPosts.length - 1) { setStoryIndex((i) => i + 1); setStoryProgress(0); }
