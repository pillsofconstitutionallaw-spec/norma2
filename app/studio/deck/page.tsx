...
export default function PdfStudioPage() {
  ...
  async function estraiTestoPDF(file: File): Promise<string> {
    ...
    let testo = '';
    const maxPagine = Math.min(pdf.numPages, 30);
        ...
        <div style={{ padding: '20px 16px 100px' }}>
          ...
          {fase === 'upload' && (
            <>
              ...
              <div onClick={() => inputRef.current?.click()} style={{ background: '#111526', border: '1.5px dashed rgba(255,255,255,0.15)', borderRadius: 20, padding: '44px 20px', textAlign: 'center', cursor: 'pointer', marginBottom: 16 }}>
                ...
                <div style={{ fontSize: 11.5, color: 'rgba(255,255,255,0.35)', lineHeight: 1.6 }}>Appunti · Dispense · Sentenze · Manuali<br />Massimo 30 pagine</div>
              </div>
              {errore && (
          ...
          {fase === 'flashcard-intro' && (
            <>
              ...
              <button onClick={() => { setIndice(0); setGirata(false); setSapute(0); setNonSapute(0); setDaRipetere([]); setFase('flashcard-studio'); }} style={{ width: '100%', padding: '16px', borderRadius: 16, border: 'none', background: 'linear-gradient(135deg, #38bdf8, #818cf8)', color: '#fff', fontWeight: 800, fontSize: 15, cursor: 'pointer', fontFamily: 'Montserrat, sans-serif' }}>
                ⚡ Inizia a studiare
              </button>
            </>
          )}
          ...
          {fase === 'flashcard-studio' && carte.length > 0 && (
            <>
              ...
              <div onClick={() => !girata && setGirata(true)} style={{ background: girata ? '#0d1f35' : '#111526', borderRadius: 24, border: `0.5px solid ${girata ? 'rgba(56,189,248,0.5)' : 'rgba(255,255,255,0.08)'}`, padding: '36px 20px', minHeight: 230, cursor: girata ? 'default' : 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', marginBottom: 20, position: 'relative', transition: 'background 0.2s, border-color 0.2s' }}>
                <div style={{ position: 'absolute', top: 14, left: 14, fontSize: 9, fontWeight: 700, letterSpacing: 2, textTransform: 'uppercase', color: girata ? '#38bdf8' : 'rgba(255,255,255,0.25)' }}>{girata ? 'RISPOSTA' : 'DOMANDA'}</div>
                {!girata && <div style={{ position: 'absolute', top: 14, right: 14, fontSize: 10, color: 'rgba(255,255,255,0.2)' }}>tocca per girare</div>}
                <div style={{ fontSize: girata ? 13 : 15, fontWeight: girata ? 500 : 700, color: '#fff', lineHeight: 1.7 }}>
                  {girata ? carte[indice].risposta : carte[indice].domanda}
                </div>
              </div>
              {girata ? (
                <div style={{ display: 'flex', gap: 10 }}>
                  <button onClick={() => rispondi(false)} style={{ flex: 1, padding: '15px', borderRadius: 16, border: '0.5px solid rgba(239,68,68,0.4)', background: 'rgba(239,68,68,0.1)', color: '#ef4444', fontWeight: 800, fontSize: 14, cursor: 'pointer', fontFamily: 'Montserrat, sans-serif' }}>✗ Non sapevo</button>
                  <button onClick={() => rispondi(true)} style={{ flex: 1, padding: '15px', borderRadius: 16, border: '0.5px solid rgba(34,197,94,0.4)', background: 'rgba(34,197,94,0.1)', color: '#22c55e', fontWeight: 800, fontSize: 14, cursor: 'pointer', fontFamily: 'Montserrat, sans-serif' }}>✓ Sapevo</button>
                </div>
              ) : (
                <div style={{ textAlign: 'center', fontSize: 12, color: 'rgba(255,255,255,0.2)', paddingTop: 4 }}>Tocca la carta per vedere la risposta</div>
              )}
            </>
          )}
